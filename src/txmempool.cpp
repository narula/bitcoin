...
void CTxMemPool::UpdateTransactionsFromBlock(const std::vector<uint256> &vHashesToUpdate)
{
    ...
    BOOST_REVERSE_FOREACH(const uint256 &hash, vHashesToUpdate) {
        ...
        // calculate children from mapSpenders
...
void CTxMemPool::pruneSpent(const uint256 &hashTx, CCoins &coins)
{
    ...
    auto it = mapSpenders.find(hashTx);
    // iterate over all the outpoints in mapSpenders whose hash equals the provided hashTx
    for (auto its = it->second.begin(); its != it->second.end(); ++its) {
        coins.Spend(its->first); // and remove those outputs from coins
    }
}
...
bool CTxMemPool::addUnchecked(const uint256& hash, const CTxMemPoolEntry &entry, setEntries &setAncestors, bool fCurrentEstimate)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
	auto it = mapSpenders.find(tx.vin[i].prevout.hash);
	auto p = std::make_pair(tx.vin[i].prevout.n, &tx);
	if (it == mapSpenders.end()) {
	    std::map<uint32_t, const CTransaction*> m;
	    m.insert(p);
	    mapSpenders.insert(std::make_pair(&tx.vin[i].prevout.hash, m));
	} else {
	    it->second.insert(p);
	}
...
void CTxMemPool::removeUnchecked(txiter it)
{
	...
	auto it = mapSpenders.find(txin.prevout.hash);
	if (it == mapSpenders.end()) {
	    continue;
	}
...
void CTxMemPool::removeRecursive(const CTransaction &origTx, std::list<CTransaction>& removed)
{
    // Remove transaction from memory pool
    {
	    ...
	    auto it = mapSpenders.find(origTx.GetHash());
	    if (it != mapSpenders.end()) {
		for (auto its = it->second.begin(); its != it->second.end(); ++its) {
		    txiter nextit = mapTx.find(its->second->GetHash());
...
void CTxMemPool::removeConflicts(const CTransaction &tx, std::list<CTransaction>& removed)
{
	...
	const CTransaction &txConflict = *its->second;
	if (txConflict != tx) {
...
void CTxMemPool::check(const CCoinsViewCache *pcoins) const
{
    ...
    LogPrint("mempool", "Checking mempool with %u transactions and %u inputs\n", (unsigned int)mapTx.size(), (unsigned int)mapSpenders.size());
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
	    ...
	    assert(mapSpenders.count(txin.prevout.hash) != 0);
            auto it3 = mapSpenders.find(txin.prevout.hash);
        ...
        // Check children against mapSpenders
        CTxMemPool::setEntries setChildrenCheck;
	auto iter = mapSpenders.find(it->GetTx().GetHash());
        int64_t childSizes = 0;
	for (auto iters = iter->second.begin(); iters != iter->second.end(); ++iters) {
            txiter childit = mapTx.find(iters->second->GetHash());
            assert(childit != mapTx.end()); // mapSpenders points to in-mempool transactions
    ...
    for (auto it = mapSpenders.cbegin(); it != mapSpenders.cend(); ++it) {
	for (auto its = it->second.cbegin(); its != it->second.cend(); ++its) {
	    uint256 hash = its->second->GetHash();
	    indexed_transaction_set::const_iterator it2 = mapTx.find(hash);
	    const CTransaction& tx = it2->GetTx();
	    assert(it2 != mapTx.end());
	    assert(&tx == its->second);
	}
    }
...
size_t CTxMemPool::DynamicMemoryUsage() const {
    ...
    return memusage::MallocUsage(sizeof(CTxMemPoolEntry) + 15 * sizeof(void*)) * mapTx.size() + memusage::DynamicUsage(mapSpenders) + memusage::DynamicUsage(mapDeltas) + memusage::DynamicUsage(mapLinks) + memusage::DynamicUsage(vTxHashes) + cachedInnerUsage;
}
            ...
            BOOST_FOREACH(const CTransaction& tx, txn) {
                BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                    if (exists(txin.prevout.hash))
                        continue;
		    if (mapSpenders.find(txin.prevout.hash) == mapSpenders.end() || mapSpenders.find(txin.prevout.hash)->second.size() == 0) {
